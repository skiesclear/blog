...
Fluid.plugins = {
  ...
  codeWidget() {
    ...
    jQuery('.markdown-body pre').each(function() {
      ...
      if ($pre.parent().hasClass('sourceCode')) {
        $pre.parent().addClass('code-wrapper');
      } else if ($pre.parent().hasClass('markdown-body') && !$pre.parent().hasClass('code-wrapper')) {
        $pre.wrap('<div class="code-wrapper"></div>');
      }
      if ($pre.find('.code-widget').length === 0) {
        $pre.append('<div class="code-widget copy-btn" data-clipboard-snippet>Copy</div>');
      }
    });
